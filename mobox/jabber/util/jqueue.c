...
void*  jqueue_next(jqueue_t q, void *node)
{
	return ((_jqueue_node_t)node)->next;
}

void  *jqueue_last(jqueue_t q)
{
	if(q->size>0)
		return q->back->data;
	else
...
void        *jqueue_iter_first(jqueue_t q)
{
	return q->back;
}

void        *jqueue_iter_next(jqueue_t q, void *node)
{
	return ((_jqueue_node_t)node)->next;
}
