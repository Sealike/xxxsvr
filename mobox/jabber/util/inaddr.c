...
int j_inet_setsock_bufsize(int sock, int size)
{
	int iOptLen = sizeof(int);

	if (setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char*)&size, iOptLen) == SOCKET_ERROR) return SOCKET_ERROR;

	if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char*)&size, iOptLen) == SOCKET_ERROR) return SOCKET_ERROR;

	return 0;
}

int j_inet_ioctl(int sock, long cmd, int arg)
{
	int iMode = arg;
#ifdef WIN32			
	return ioctlsocket(sock, cmd, &iMode);
#else
	return ioctl(sock, cmd, &iMode);
#endif
}