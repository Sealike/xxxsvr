...
char *j_str_gbk2utf8(char *src, int srcl, char *dst, int dstl)
{
	char *out = NULL, *res = NULL;
#ifdef _WIN32
	int length = MultiByteToWideChar(CP_ACP, 0, src, -1, NULL, NULL);
	out = (char *)malloc(length * sizeof(WCHAR) + 1);
	out[length] = 0;
	MultiByteToWideChar(CP_ACP, 0, src, -1, out, length);

	length = WideCharToMultiByte(CP_UTF8, 0, out, -1, NULL, 0, NULL, NULL);

	WideCharToMultiByte(CP_UTF8, 0, out, -1, dst, (dstl > length ? length : dstl), NULL, NULL);
	free(out);
	return dst;
#else
	return src;
...
int64_t j_atoi64(const char *a)
{
	if(NULL == a) return 0;
#ifdef _WIN32
	return _strtoi64(a, NULL, 10);
#else
	return strtoll(a, NULL, 10);
#endif
}
