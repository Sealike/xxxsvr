...
avl_create (avl_comparison_func *compare, void *param,
            struct libavl_allocator *allocator, mempool_t mp)
{
...
avl_probe (struct avl_table *tree, void *item)
{
  ...
  if(NULL != tree->avl_mp)
	  n = q->avl_link[dir] = allocate(tree->avl_mp, sizeof(*n));
  else
	  n = q->avl_link[dir] =
          ...
          avl_item_func *destroy, struct libavl_allocator *allocator)
{
  ...
  new = avl_create(org->avl_compare, org->avl_param,
                    allocator != NULL ? allocator : org->avl_alloc, NULL);
  if (new == NULL)
    return NULL;
...
/* Frees storage allocated for all |node|.
   If |destroy != NULL|, applies it to each data item in inorder. */
void
avl_clear (struct avl_table *tree, avl_item_func *destroy)
{
  struct avl_node *p, *q;
