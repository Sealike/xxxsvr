...
#endif
#define fd_array_copy(dest,src) \
	{\
		struct fd_set* srcS = (struct fd_set*)src;\
		struct fd_set* dstS = (struct fd_set*)dest;\
		dstS->fd_count = src->fd_count;\
		memcpy(dstS->fd_array,srcS->fd_array,sizeof(dstS->fd_array[0])*dstS->fd_count);\
	}
...
#define MIO_FUNCS \
    ...
    {                                                                   \
		...
		fd_array_copy((&m->rfds_out),(&m->rfds_in));                    \
		fd_array_copy((&m->wfds_out),(&m->wfds_in));                    \
        tv.tv_sec  = t;                                                 \
        tv.tv_usec = 0;                                                 \
