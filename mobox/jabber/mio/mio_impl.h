...
static void _mio_write(mio_t m, mio_fd_t fd)
{
	...
	mio_debug(ZONE, "当前socket状态[0x%02x]", FD(m,fd)->type);
    /* if connecting, do this later */
    if(FD(m,fd)->type & type_CONNECT)
    {
        FD(m,fd)->type |= type_CONNECT_WRITE;
		mio_debug(ZONE, "现在socket状态[0x%02x]", FD(m,fd)->type);
        return;
    }
...
static mio_fd_t _mio_connect(mio_t m, int port, char *hostip, mio_handler_t app, void *arg)
{
    ...
    if(flag == 0)
    {
	    /* set the socket to non-blocking after connecting */
#if defined(HAVE_FCNTL)
		flags = fcntl(fd, F_GETFL);
	    flags |= O_NONBLOCK;
 	   fcntl(fd, F_SETFL, flags);
...
/** set up a udp listener in this mio w/ this default app/arg */
static mio_fd_t _mio_udp_listen(mio_t m, int port, char *sourceip, mio_handler_t app, void *arg)
{
    int fd, flag = 1;
    mio_fd_t mio_fd;
    struct sockaddr_storage sa;
    ...
    /* attempt to create a socket */
    if((fd = socket(sa.ss_family,SOCK_DGRAM,0)) < 0) return NULL;
    if(setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (char*)&flag, sizeof(flag)) < 0) return NULL;
...
static mio_t _mio_new(int maxfd)
{
    static struct mio_st mio_impl = {
        _mio_free,
        _mio_listen, _mio_connect, _mio_udp_listen, _mio_setup_fd,
        _mio_app,
        _mio_close,
