...
typedef struct mio_st
{
  ...
  struct mio_fd_st *(*mio_udp_listen)(struct mio_st **m, int port, char *sourceip,
				  mio_handler_t app, void *arg);
...
#define mio_udp_listen(m, port, sourceip, app, arg) \
    (*m)->mio_udp_listen(m, port, sourceip, app, arg)
