...
typedef struct tagUSET_TXSQ
{
	...
	UINT16  Len;				// 电文长度(Byte)//当Flag指示为1及代码方式时，该长度为bit位数。
	char	Msg[421];			// 电文内容;
}USET_TXSQ,USET_TXA;
...
#ifndef LGET_WZBG
#define	LGET_WZBG LSET_WZBG		// A0位置报告
#endif	

#ifndef LGET_YQBG
#define	LGET_YQBG LSET_YQBG		// A2要求报告
#endif

#ifndef LGET_TSSJ
#define	LGET_TSSJ LSET_TSSJ		// A3态势信息
#endif

#ifndef LGET_ZYDW
#define	LGET_ZYDW LSET_ZYDW		// A4自由电文
#endif
...
typedef struct tagUGET_ZDA
{
	...
	UINT8   T[4];			// UTC时间
	UINT8	D[3];			// UTC日期
	UINT8	LocalH;			// 本地时-HH(-13~+13,负=东经，正=西经）
...
typedef struct tagUGET_TXXX
{
	...
	UINT16	Len;			// 长度（字节)//Flag指示为1代码方式时，单位为bit数。
...
{   
	...
	UINT8 nFrec; // 1=B1,2=B1B3,3=B3,4=B3L1G1,5=L1,6=G1,7=B1L1; 1 B3; 3B1
