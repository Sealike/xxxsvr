...
{
	client->moniclient_fd = mio_listen(client->mio, client->moniclient_port, client->moniclient_ip, _p2p_mio_callback, (void *)client);
	if (client->moniclient_fd == NULL) {
		g_shutdown = TRUE;
		log_write(client->log, LOG_ERR, "[%s, port=%d @%d] failed to listen moniclient.", client->moniclient_ip, client->moniclient_port, client->moniclient_fd->fd);
	}else
		log_write(client->log, LOG_NOTICE, "[%s, port=%d @%d] listening for moniclient connections", client->moniclient_ip, client->moniclient_port, client->moniclient_fd->fd);
}
...
static int _p2p_mio_callback(mio_t m, mio_action_t a, mio_fd_t fd, void *data, void *arg) 
{
	...
	case action_CLOSE:
		...
		// 将该sess从其订阅的所有taskid对应的队列中删除
		clear_sess_from_eachtaskid(sess);
	...
	case action_ACCEPT:
		...
		if (port == client->moniclient_port){
