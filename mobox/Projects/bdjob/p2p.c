...
void p2p_listener_start(instapp_t client) 
{
	...
	log_write(client->log, LOG_ERR, "[%s, port=%d] failed to listen front end.", client->frontend_ip, client->frontend_port);
	}else
	log_write(client->log, LOG_NOTICE, "[%s, port=%d] listening for front end connections", client->frontend_ip, client->frontend_port);
	...
	log_write(client->log, LOG_ERR, "[%s, port=%d] failed to listen front end.", client->backend_ip, client->backend_port);
	}else
	log_write(client->log, LOG_NOTICE, "[%s, port=%d] listening for front end connections", client->backend_ip, client->backend_port);*/
}
...
static int _p2p_mio_callback(mio_t m, mio_action_t a, mio_fd_t fd, void *data, void *arg) 
{
	...
	int namelen = sizeof(sa), nbytes, flags = 0;
	...
	case action_ACCEPT:
		...
		//if(port == client->backend_port){					
		//	p2p->sess.type = back_SESS;
		//	p2p->expat = back_exp_new(&p2p->sess);
		//}else if(port == client->frontend_port){		
		//	p2p->sess.type = front_SESS;
		//	p2p->expat = dev_exp_new(&p2p->sess);
		//	//xhash_walk(client->namedend,xhash_walk_throgh_2,NULL);
		//}
