...
void _p2pclient_init(p2pclient_t p2p) 
{
	...
	p2p->want_read = 1;
	protobuffer_send(p2p,eProto_initsession,NULL); //与服务器连接
	log_write(g_instapp->log, LOG_NOTICE, "send initsession to bdsvr.");
...
BOOL p2pclient_connect(p2pclient_t p,mio_t mio,jqueue_t deadsess,void* param,sess_type_t sesstype,io_complete_cb iccb,void* para)
{
	...
	if(p->sess.fd == NULL){
		if(logger()) log_write(logger(), LOG_ERR, "[%s, port=%d] failed to connect to.", p->ip, p->port);
		jqueue_push(deadsess,p,0);
		p->sess.binitsess = FALSE;
		p->lost = TRUE;
		return FALSE;
	}else{
		p->mio = mio;
		...
		if(logger()) log_write(logger(), LOG_NOTICE, "[ip=%s, port=%d] success connect to %s.", p->ip, p->port,(cnt==0)?"bdsvr":p->sess.sname);
		p->lost = FALSE;
		p->client = instapp;
		p->sess.instapp = instapp;
		p->sess.binitsess = FALSE;//初始化
		p->iccb = iccb;
		p->para = para;
