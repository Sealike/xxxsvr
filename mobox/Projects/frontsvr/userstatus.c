...
void  users_act_save(users_act_t us, char* key, int sec, int type, void *arg)
{
	...
	user_status_arg_t us_arg = (user_status_arg_t)arg;
	strcpy_s(s->arg.executor, strlen(us_arg->executor)+1, us_arg->executor);
	strcpy_s(s->arg.lon, strlen(us_arg->lon)+1, us_arg->lon);
	strcpy_s(s->arg.lat, strlen(us_arg->lat) + 1, us_arg->lat);
	strcpy_s(s->arg.hgt, strlen(us_arg->hgt) + 1, us_arg->hgt);
}
