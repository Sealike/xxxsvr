...
void _p2pclient_release(p2pclient_t p2p)
{
	if(p2p){
		...
		if (p2p->expat) exp_clear(p2p->expat);
...
BOOL p2pclient_connect(p2pclient_t p,mio_t mio,jqueue_t deadsess,void* param,sess_type_t sesstype,io_complete_cb iccb,void* para)
{
	...
	if(p->sess.fd == NULL){
		...
		jqueue_push(client->dead_sess,p,0);
		p->lost = TRUE;
		...
		if (logger()) log_write(logger(), LOG_NOTICE, "[%s, port=%d] success connect to %s.", p->ip, p->port,p->sess.sname);
		p->lost = FALSE;
		p->sess.binitsess = FALSE;  // 无论是否连接成功，都先将binitsess置为FALSE
		p->client = p->sess.client = client;
...
int p2pclient_write(p2pclient_t p2p, response_pkt_p packet) {
	...
	if(p2p->mio == NULL || p2p->sess.fd == NULL)
		return -1;
...
static int _p2pclient_can_read(p2pclient_t p2p) 
{
	...
	if (len > 100)
	{
		...
		//printf("in _p2pclient_can_read(),recv_buffer = %s", buffer);
	}
...
static int _p2pclient_can_write(p2pclient_t p2p)
{
	...
	if(written < 0) {
		...
		response_pkt_free(out);
		p2p->want_write = 0;
		p2p->want_read = 0;		// recently modify
		_p2pclient_kill(p2p);
		return 0;
...
void p2pclient_init(p2pclient_t p2p, init_sess_t initparam)
{
	...
	protobuffer_send(p2p, eProto_initsession, initparam); //与服务器连接
}
