...
//	int i = 0;
//
//	if (dbinst == NULL || conf == NULL || dbinst->m_con == NULL)
//	{
//		log_write(client()->log, LOG_ERR, "db_query_config, error: query == NULL || query->m_con == NULL, return -1.", sql);
//		return -1;
//	}
//
//	sprintf(sql, "SELECT * FROM t_pconfig");
//
...
//		{
//			if (dbinst->m_row[0])
//				conf->warntime = atoi(dbinst->m_row[0]);
//			rt = 0;
//			break;
//		}
...
//	}
//	else{
//		log_write(client()->log, LOG_ERR, "db_query_config. error: %s.", mysql_error(dbinst->m_con));
//	}
//
//	return rt;
//}
...
int db_query_config(mysqlquery_t dbinst, int * warntime)
{
	...
	if (dbinst == NULL || warntime == NULL || dbinst->m_con == NULL)
	{
		log_write(client()->log, LOG_ERR, "db_query_config, error: query == NULL || query->m_con == NULL, return -1.", sql);
		return -1;
	}
...
//	if (dbinst == NULL || conf == NULL || dbinst->m_con == NULL)
//	{
//		log_write(client()->log, LOG_ERR, "db_update_config, error: query == NULL || query->m_con == NULL, return -1.", sql);
//		return -1;
//	}
//
//	sprintf(sql, "UPDATE t_pconfig SET warntime=%d",conf->warntime);
//
//	log_write(client()->log, LOG_NOTICE, "SQL = [%s].", sql);
//
...
//		log_write(client()->log, LOG_NOTICE, "db_update_config. OK: affected rows = %d.", affectedrows);
//	}
//	else{
//		log_write(client()->log, LOG_ERR, "db_update_config. error: %s.", mysql_error(dbinst->m_con));
//
//		affectedrows = -1;
//	}
//
...
int db_update_config(mysqlquery_t dbinst, int warntime)
{
	...
	if (dbinst == NULL || warntime == 0 || dbinst->m_con == NULL)
	{
		log_write(client()->log, LOG_ERR, "db_update_config, error: query == NULL || query->m_con == NULL, return -1.", sql);
		return -1;
	}

	sprintf(sql, "UPDATE t_pconfig SET warntime=%d", warntime);
...
int db_save_plog(mysqlquery_t dbinst, plog_t plog)
{
	...
	if (dbinst == NULL || plog == NULL || dbinst->m_con == NULL)
	{
		log_write(client()->log, LOG_ERR, "db_save_log, error: query == NULL || query->m_con == NULL, return -1.", sql);
		return -1;
	}

	sprintf(sql, "INSERT INTO t_plog(taskid,type,time,source,net,executor,lon,lat,hgt,`desc`) VALUES(%d,'%s','%s','%s','%s','%s','%s','%s','%s','%s')", plog->taskid, plog->type,plog->time, plog->source, plog->net, plog->executor, plog->lon, plog->lat, plog->hgt,plog->desc);
	...
	if (rt == 0){
		affectedrows = mysql_affected_rows(dbinst->m_con);
		log_write(client()->log, LOG_NOTICE, "db_save_plog. OK: affected rows = %d.", affectedrows);
	}
	else{
		log_write(client()->log, LOG_ERR, "db_save_plog. error: %s.", mysql_error(dbinst->m_con));
...
int db_save_status(mysqlquery_t dbinst, status_t status)
{
	...
	if (dbinst == NULL || status == NULL || dbinst->m_con == NULL)
	{
		log_write(client()->log, LOG_ERR, "db_save_status, error: query == NULL || query->m_con == NULL, return -1.", sql);
		return -1;
	}

	sprintf(sql, "INSERT INTO t_status(taskid,beam,battery, time,source,net,executor,lon,lat,hgt) VALUES(%d,'%s','%s','%s','%s','%s','%s','%s','%s','%s')", status->taskid, status->beam, status->battery, status->time, status->source, status->net, status->executor, status->lon, status->lat, status->hgt);
	...
	if (rt == 0){
		affectedrows = mysql_affected_rows(dbinst->m_con);
		log_write(client()->log, LOG_NOTICE, "db_save_status. OK: affected rows = %d.", affectedrows);
	}
	else{
		log_write(client()->log, LOG_ERR, "db_save_status. error: %s.", mysql_error(dbinst->m_con));
...
int db_save_boxpos(mysqlquery_t dbinst, boxpos_t boxpos)
{
	...
	if (dbinst == NULL || boxpos == NULL || dbinst->m_con == NULL)
	{
		log_write(client()->log, LOG_ERR, "db_save_boxpos, error: query == NULL || query->m_con == NULL, return -1.", sql);
		return -1;
	}

	sprintf(sql, "INSERT INTO t_position(taskid,time,source,net,executor,lon,lat,hgt) VALUES(%d,'%s','%s','%s','%s','%s','%s','%s')", boxpos->taskid, boxpos->time, boxpos->source, boxpos->net, boxpos->executor, boxpos->lon, boxpos->lat, boxpos->hgt);
	...
	if (rt == 0){
		affectedrows = mysql_affected_rows(dbinst->m_con);
		log_write(client()->log, LOG_NOTICE, "db_save_boxpos. OK: affected rows = %d.", affectedrows);
	}
	else{
		log_write(client()->log, LOG_ERR, "db_save_boxpos. error: %s.", mysql_error(dbinst->m_con));
...
int db_save_warning(mysqlquery_t dbinst, warning_t warning)
{
	...
	if (dbinst == NULL || warning == NULL || dbinst->m_con == NULL)
	{
		log_write(client()->log, LOG_ERR, "db_save_warning, error: query == NULL || query->m_con == NULL, return -1.", sql);
		return -1;
	}

	sprintf(sql, "INSERT INTO t_warning(taskid,type,time,source,net,executor,lon,lat,hgt) VALUES(%d,'%s','%s','%s','%s','%s','%s','%s','%s')", warning->taskid, warning->type, warning->time, warning->source, warning->net, warning->executor, warning->lon, warning->lat, warning->hgt);
	...
	if (rt == 0){
		affectedrows = mysql_affected_rows(dbinst->m_con);
		log_write(client()->log, LOG_NOTICE, "db_save_warning. OK: affected rows = %d.", affectedrows);
	}
	else{
		log_write(client()->log, LOG_ERR, "db_save_warning. error: %s.", mysql_error(dbinst->m_con));
...
int db_auth_excecutor(mysqlquery_t dbinst, char *name, char *taskid, char ** pwd)
{
	...
	if (dbinst == NULL || dbinst->m_con == NULL || name == NULL || pwd == NULL)
	{
		log_write(client()->log, LOG_NOTICE, "db_auth_excecutor. error: dbinst == NULL || dbinst->m_con == NULL.");
		return -1;
	}

	sprintf(sql, "SELECT p.password FROM t_task t JOIN t_task_executor e ON t.id=e.task_id JOIN t_person p ON e.person_id=p.id  WHERE p.code=%s AND t.id=%s", name,taskid);
	rt = mysql_real_query(dbinst->m_con, sql, strlen(sql));
	...
	if (rt == 0)
	{
		...
		rt = -1;
		printf("db_auth_excecutor . query error :%s", mysql_error(dbinst->m_con));
	}
...
int db_auth_excecutor_simple(mysqlquery_t dbinst, char *name, char *taskid, char ** pwd)
{
	char sql[512] = { 0 };
	int  rt = 0;
	int printn = 0;
	...
	printn += sprintf(sql + printn, "SELECT password  FROM t_person where code=%s", name);
	//sprintf(sql, "SELECT p.password FROM t_task t JOIN t_person p ON t.executeid=p.id WHERE p.code=%s AND t.id=%s", name, taskid);
	rt = mysql_real_query(dbinst->m_con, sql, strlen(sql));
