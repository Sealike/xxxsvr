	...
	case cunpack_initsession:
		{
			...
			if(strncmp(sess->role,"bdsvr",sizeof("bdsvr")) == 0)
			{
				bdsvr_exp_t bdexp = bdsvr_exp_new(sess);
				exp_set_callback((_exp_t)bdexp, bdsvr_exp_cb);
				opensess_exp_free(exp);
				p2p->expat = (_exp_t)bdexp;
				sess->state = state_init_SESS;
				sess->auth_cnt = 0;//初始化认证次数
				sess->usertype = -1;
				rt = -1;
			}
			else if (strncmp(sess->role, "monitor", sizeof("monitor")) == 0)
			{
				monitor_exp_t monitorexp = monitor_exp_new(sess);
				exp_set_callback((_exp_t)monitorexp, monitor_exp_cb);
				opensess_exp_free(exp);
			...
			else if (strncmp(sess->role, "app", sizeof("app")) == 0)
			{	
				// 因为协议格式基本一致，app和bdsvr共用一套解析器
				bdsvr_exp_t bdexp = bdsvr_exp_new(sess);
				exp_set_callback((_exp_t)bdexp, bdsvr_exp_cb);
				opensess_exp_free(exp);
				p2p->expat = (_exp_t)bdexp;
			...
			strcpy_s(initack.from, sizeof(initack.from) - 1, "frontsvr");
			strcpy_s(initack.to, sizeof(initack.to) - 1, jrole->valuestring);
