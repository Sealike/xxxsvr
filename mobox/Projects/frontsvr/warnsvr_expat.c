...
static int search_frame(warnsvr_exp_t explain, unsigned char*p, int len, int* searchpos);
static int  explain(warnsvr_exp_t exp, uchar*frame, int len);
...
static int search_frame(warnsvr_exp_t explain, unsigned char*p, int len, int* searchpos)
{
	...
	for (; dwCurrPt < len; dwCurrPt++)
	{
		...
		case SEARCHMACHINE_NULL:	//  0: $
		{
			...
			{//搜索头部
				int iistileid = istitle(exp->Head);
				if (iistileid)
				{

					int c = 0;
					iHeadLen = strlen(MSGTitle[iistileid - 1].title);
		...
		case SEARCHMACHINE_IDENTIFIER:
		{
			...
			if (ch == '\n' && exp->frame[exp->framelen - 2] == '\r' && exp->frame[exp->framelen - 5] == '*')
			{//
				if (dwCurrPt == (len - 1))
...
{
	warnsvr_exp_t rtn = NULL;

	while ((rtn = (warnsvr_exp_t)calloc(1, sizeof(warnsvr_exp_st))) == NULL);
...
void    warnsvr_exp_free(warnsvr_exp_t exp)
{
	if (exp) free(exp);
}
