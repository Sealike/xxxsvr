...
//目的地设定常量定义
typedef struct tagMDSDInfo
{
	int nLatDegree;//纬度 度  
	int nLatMinute;//纬度 分
	float fLatSecond;//纬度 秒 精度：0.1秒
	int nLongDegree;//经度 单位：度
	int nLongMinute;//经度 单位：分
	float fLongSecond;//经度，精度：0.1秒
	BYTE byCoordType;//坐标类型
	BYTE byOrient;//方向指示
	char chDescription[1024];//描述
}MDSDInfo,*lpMDSDInfo;

//路径规划常量定义
typedef struct tagLJGHInfo
{
	char chRouteType;//路径类型
	int  nPoints;//点数
	int nLatDegree;//纬度 度  
	int nLatMinute;//纬度 分
	float fLatSecond;//纬度 秒 精度：0.1秒
...
//状态上报指令常量定义
typedef struct tagZTSBInfo
{
	int nRepFreq;//报告频度
}ZTSBInfo,*lpZTSBInfo;

//数据传输指令常量定义
typedef struct tagSJCSInfo
{
	BYTE byTransType;//传输类型
	char chAddress[256];//地址
	char chStartTime[14];//开始时间
	char chTerminalTime[14];//结束时间
	UINT chSampleInterval;//采样时间间隙
	char chContent[1024];
}SJCSInfo,*lpSJCSInfo;

//指令响应常量定义
typedef struct tagZLXYInfo
{
	int nLatDegree;//纬度 度  
	int nLatMinute;//纬度 分
	float fLatSecond;//纬度 秒 精度：0.1秒
	...
	int nLongMinute;//经度 单位：分
	float fLongSecond;//经度，精度：0.1秒
	unsigned char  byOrient;//方向指示
	int nBatteryCap;//电池容量
}ZLXYInfo,*lpZLXYInfo;

//用户机状态上报常量定义
typedef struct tagUZTSBInfo
{
	int nRepFreq;//报告频度
	int nLatDegree;//纬度 度  
	int nLatMinute;//纬度 分
...
{   
	DWORD LocalID;
	int  ifUrgent;		//0:否  1:紧急定位
	int  ifTimeDiff;	//有无时差[固定填0]
	BYTE  MeasureMode;	//00:有高程  01:无测高  10:测高1  11:测高2
	int  ifSecret;		//保密[固定填零]

	int  ifHighAlt;	//[标准协议2.1版无此项] 高程指示[0:普通  1:高空]
...
{   	
	unsigned char nFrec; // 1=B1,2=B1B3,3=B3,4=B3L1G1,5=L1,6=G1,7=B1L1
